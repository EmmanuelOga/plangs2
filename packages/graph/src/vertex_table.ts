 ...
 */
export class VertexTable<T_VID extends string, T_VData extends VData<T_VID>>
	implements Iterable<[T_VID, T_VData]>
{
	...
	set(vid: T_VID, value: T_VData): this {
		if (!this.validParams(vid))
			throw new Error(`invalid vid: ${vid} (pattern: ${this.vidPattern})`);
