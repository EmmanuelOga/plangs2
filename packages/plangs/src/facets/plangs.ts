...
export const PLANG_FACET_PREDICATES = {
  ...
  influenced: (({ relInfluenced }, flt) => flt.matches(key => relInfluenced.has(key))) as Pred<Filter<VPlang["key"]>>,
  ...
  paradigms: (({ relParadigms }, flt) => flt.matches(key => relParadigms.has(key))) as Pred<Filter<VParadigm["key"]>>,
  ...
  typeSystems: (({ relTypeSystems }, flt) => flt.matches(key => relTypeSystems.has(key))) as Pred<Filter<VTypeSystem["key"]>>,
  writtenIn: (({ relWrittenWith }, flt) => flt.matches(key => relWrittenWith.has(key))) as Pred<Filter<VPlang["key"]>>,
} as const;
