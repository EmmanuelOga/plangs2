...
import { cleanup, tap } from "@plangs/auxiliar/misc";
...
export abstract class PlangsVertex<KeyPrefix extends string, Data extends VertexBaseData> extends Vertex<KeyPrefix, Data> {
  ...
  get detail(): VertexDetail {
    ...
    if (vertex.urlHome) general.push({ kind: "link", href: vertex.urlHome, value: "Home" });
  ...
  toCode(): string {
    ...
    for (const [relName, relation] of vertex.relations) {
      ...
      const keys = [...relation.keys].sort();
      if (keys.length > 0) relations.push(`\n    .${relName}.add(${JSON.stringify(keys).slice(1, -1)})`);
    }

    const setter = `g.${vertex.vertexName}.set(${JSON.stringify(vertex.key)}, ${JSON.stringify(data)})${relations.join("")}`;
