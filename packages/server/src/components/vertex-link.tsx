...
export function VertexLink({ vertex, title, includeLocal }: { vertex: TPlangsVertexClass; title?: string; includeLocal: boolean }) {
  const url = includeLocal ? vertex.href : vertex.urlHome;
  const overrideProse = { style: "color: var(--color-foreground);" };
  return (
