...
const config = {
  ...
  processors: [
    ...
    (entry: string) => entry.replace(config.punctuationRE, ""),
    (entry: string) => {
...
const loadSaveData = (str: string) => {
  const arr: string[] = [];
  const stack: string[][] = [arr];
  let currentIndex = 1;

  while (stack.length !== 0) {
    const currentArr: any[] = stack[stack.length - 1];
    let element = "";
    ...
    for (; currentIndex < str.length; currentIndex++) {
      ...
      if (char === ",") {
        if (element.length !== 0) {
          if (element[0] === "@") {
            const elementInt = Number.parseInt(element.substring(1));
            for (let i = 0; i < elementInt; i++) {
              currentArr.push(undefined);
            }
          } else {
            currentArr.push(Number.parseFloat(element));
          }
          element = "";
        }
      } else if (char === "[") {
    ...
    if (element.length !== 0) {
      currentArr.push(Number.parseInt(element));
    }
  }
...
const update = (results, resultIndexes, increment, data) => {
...
export function Wade(data): { (query: string): never[]; index: any } {
  const search = (query) => {
  ...
  if (Array.isArray(data)) {
    ...
    search.index = loadSaveData(data);
  }
...
Wade.index = (data) => {
      ...
      for (let j = 0; j < termsLength; j++) {
        ...
        for (let n = 0; n < term.length; n++) {
          ...
          if (highByte !== 0) {
            ...
            const nextRanges = currentRanges[highByte];
  ...
  const nodeDataSets = [];
  const index = new Array(indexSize);
  index[0] = [indexOffset];
...
Wade.save = (search) => stringify(search.index);
