...
> implements Iterable<[EdgeKey, T_EdgeData]>
{
	...
	get(
		...
		to: T_Id_V_To,
		suffix?: string,
	): T_EdgeData | undefined {
		if (!this.validParams(from, to, suffix))
			throw new Error(
				`Invalid id(s) in edge: ${from} -> ${to}${s ? ` ${s}` : ""}.`,
			);
	...
	validParams(from: T_Id_V_From, to: T_Id_V_To, suffix?: string): boolean {
		return (
			this.from_t.validParams(from) &&
			this.to_t.validParams(to) &&
			(!suffix || validChars(suffix))
		);
	}
	...
	*adjacentTo(to: T_Id_V_To, suffix?: string) {
		...
		if (!data) return;
		const adjSet =
			(this.directed ? data.adjTo.get(to) : data.adjFrom.get(to)) ?? [];
