...
export class Graph {
	...
	protected v_table<T_VData, T_VId extends string>(
		type: string,
	): VertexTable<T_VId, T_VData> {
	...
	protected e_table<
		T_EdgeData,
		T_VId_From extends string,
		T_VId_To extends string,
	>(
		type: string,
		...
		const table = new EdgeTable<T_VId_From, T_VId_To, _Any>(
			type,
			from,
			to,
			directed === "directed",
